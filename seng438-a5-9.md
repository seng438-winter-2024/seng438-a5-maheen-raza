...
Above, I used excel in order to plot the time-between-failures graph, using the chapter 10 slides from lecture as reference. In order to graph the plot, I used the "failure time" column in the failure report 2 word document as the "cumulative time between failures" data on the x-axis, and calculated the "failure per second" as error number over the the failure time in order to get data for the y-axis.
