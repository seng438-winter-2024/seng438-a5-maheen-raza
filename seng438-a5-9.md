...
For the plot above, I also used excel in order to graph the time-between-failures graph, by using the "failure number" column in failure report 2 as the x-axis, and the "time since last failure" column as the y-axis. Based on the peaks of this graph, this indicates that as the number of failures increase, the time between them starts to increase. 
